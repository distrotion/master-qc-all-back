...
router.post('/GET_ITEMSget_FINAL', async (req, res) => {
  ...
  let findCALCULATECON = await mongodb.find(headers['server'],masterDB, CALCULATECON, { "activeid": "active_id" });
  ...
  if (input.masterID !== undefined) {
    ...
    if (input.masterID === '') {
      ...
      if (find01.length > 0) {
        ...
        if (find02.length > 0) {
          let update01 = await mongodb.update(headers['server'],masterDB, SPECIFICATION, { 'masterID': uid }, { "$set": { "ITEMs": input[`ITEMs`],"SPECIFICATION": input[`SPECIFICATION`]} });
          output = "OK";
        } else {
          console.log(input);
...
router.post('/EDIT_CALCULATECON_FINAL', async (req, res) => {
  ...
  if (input.masterID !== undefined) {
    ...
    if (input.masterID === '') {
      let find02 = await mongodb.find(headers['server'],masterDB, CALCULATECON, { "CALCULATECON": input[`CALCULATE`], "activeid": "active_id" });
      ...
      if(find02.length>0){
        ...
        let insert01 = await mongodb.insertMany(headers['server'],masterDB, CALCULATECON, [input]);
        output = "OK";
      }
      

    } else {
      let find01 = await mongodb.find(headers['server'],masterDB, CALCULATECON, { "masterID": input[`masterID`] });
      ...
      if (find01.length > 0) {
        ...
        if (find02.length > 0) {
          ...
          let update01 = await mongodb.update(headers['server'],masterDB, CALCULATECON, { 'masterID': uid }, { "$set": input });
          output = "OK";
        } else {
          let update01 = await mongodb.update(headers['server'],masterDB, CALCULATECON, { 'masterID': uid }, { "$set": input });
          output = "OK";
...
router.post('/GET_UNIT_ITEM', async (req, res) => {
  ...
  if (input['ITEMs'] !== undefined) {
    ...
    let findITEMs = await mongodb.find(headers['server'],masterDB, ITEMs, { "activeid": "active_id" , "masterID": input[`ITEMs`] });
    //UNIT
    
   if(findITEMs.length>0){
    let findUNIT = await mongodb.find(headers['server'],masterDB, UNIT, { "activeid": "active_id" , "TYPE": findITEMs[0]['TYPE'] });
